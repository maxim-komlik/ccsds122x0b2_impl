...
template<typename T, size_t alignment>
SegmentAssembler<T, alignment>::output_type SegmentAssembler<T, alignment>::apply(subbands_t<T>&& buffers) {
	output_type output = this->pack(buffers);
	for (size_t i = 1; i < buffers.size(); ++i) {
		// free buffers, they are not necessary once we packed all values
		buffers[i] = bitmap<T, alignment>();
	}

	{
		...
		for (auto& item : output) {
			...
			for (ptrdiff_t j = 0; j < item->size; ++j) {
				std::make_unsigned_t<oT> magnitude_mask = accorv<oT, alignment>(item->data[j].content, constants::block::items_per_block);
			...
			// item->bdepthAc = bdepthv<T, alignment>(item->data.data()->content, item->size * constants::block::items_per_block);
			dc_index += item->size;
		}
		buffers[0] = bitmap<T, alignment>();
	}
...
template <typename T, size_t alignment>
SegmentAssembler<T, alignment>::output_type SegmentAssembler<T, alignment>::pack(subbands_t<T>& buffers) {
	// Does not pack DC coefficients into blocks, AC only

	img_meta DC_loc = buffers[0].get_meta();
	size_t block_count = (DC_loc.width * DC_loc.height);
	size_t target_segment_size = this->segment_size;
	...
	for (size_t i = 0; i < DC_loc.height; ++i) {
		for (size_t j = 0; j < DC_loc.width; ++j) {
			...
			for (size_t k = 0; k < constants::subband::families_num; ++k) {
				size_t offset = 1;
				size_t index = 0;
				size_t size = 1;
				size_t stride = 1;
				current.content[k + offset] = 
					this->handle_item_pack(buffers[k + 1][i][j]);
				...
				for (size_t ii = 0; ii < 2; ++ii) {
					for (size_t jj = 0; jj < 2; ++jj) {
						T& coeff = buffers[k + 4][i * stride + ii][j * stride + jj];
...
template <typename T, size_t alignment>
void SegmentAssembler<T, alignment>::set_segment_size(size_t S) {
	bool valid = true;
	valid &= S <= constants::segment::max_blocks_per_segment;
	valid &= S >= constants::segment::min_blocks_per_segment;
	if (!valid) {
...
template <typename T, size_t alignment>
SegmentDisassembler<T, alignment>::output_type SegmentDisassembler<T, alignment>::apply(std::vector<std::unique_ptr<segment<iT>>>&& input) {
	bool valid = true;
	valid &= this->img_width > 0;
	if (!valid) {
		// TODO: validation error
	}
	...
	for (ptrdiff_t i = 0; i < input.size(); ++i) {
		...
		// 
		// T mask = ~(-1 << (input[i]->bdepthDc - input[i]->q));
		...
		// 
		// 	diffData[j] = diffData[j - 1] + diff;
		// 	input[i]->data[j + 1].content[0] |= diffData[j] << input[i]->q;
		// }
		...
		size_t bdepthAcBdepth = std::bit_width(input[i]->bdepthAc);
		if (bdepthAcBdepth > 1) {
			ptrdiff_t* diffBdepthAc = (ptrdiff_t*)input[i]->quantizedBdepthAc.data();
			diffBdepthAc[-1] = input[i]->referenceBdepthAc;
			rkdiff<ptrdiff_t, alignment>(diffBdepthAc, (input[i]->size - 1), bdepthAcBdepth, true);
		}
	}

	return this->unpack(std::move(input));
}
...
SegmentDisassembler<T, alignment>::output_type SegmentDisassembler<T, alignment>::unpack(std::vector<std::unique_ptr<segment<iT>>>&& input) {
	...
	output_type buffers_collection;
				...
				for (ptrdiff_t bound_i = 0; bound_i <= 6; bound_i += 2) {
					...
					for (; l < bound; ++l) {
						...
						buffers_collection.back()[3 * relu(level - 1) + disp[0]]
							...
							= this->handle_item_unpack(input[i]->data[j].content[l]);
					}
				}
...
template <typename T, size_t alignment>
void SegmentDisassembler<T, alignment>::set_image_width(size_t width) {
	bool valid = true;
...
template <typename T, size_t alignment>
size_t SegmentDisassembler<T, alignment>::get_image_width() const {
...
template class SegmentAssembler<int8_t>;
template class SegmentAssembler<int16_t>;
template class SegmentAssembler<int32_t>;
...
template class SegmentDisassembler<int8_t>;
template class SegmentDisassembler<int16_t>;
template class SegmentDisassembler<int32_t>;
