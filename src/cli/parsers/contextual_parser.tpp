...
private:
	...
	std::array<bool, immediates_size + named_size> processed = { false };
...
public:
	...
	void parse(std::vector<std::string_view>& tokens) {
		constexpr size_t total_items_num = immediates_size + named_size;
		...
		if (std::any_of(this->processed.cbegin(), this->processed.cend(),
			...
			std::invoke(
				...
				std::make_index_sequence<total_items_num>());
		}
	}
...
private:
	void parse_impl(std::vector<std::string_view>& tokens) {
		auto handle_parsing_error = [&]<size_t index>(const cli::unexpected& reason) {
			...
			case std::errc::result_out_of_range: // TODO: but code intended to be used for operation results only?
	...
	bool try_parse_special_tokens(std::vector<std::string_view>& tokens) {
		...
		if (token == global_context::help.name) {
			...
			if (index < immediates_size + named_size) {
